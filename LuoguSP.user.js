...
(function () {
	...
	// 首次运行：所有功能默认开启
	for (const k of FEATURE_LABELS.keys()) if (!storage.has(k)) storage.set(k, true);
	...
	function addSettingButton() {
		...
		for (const sel of SELECTORS.navContainers) { const n = document.querySelector(sel); if (n) { nav = n; navSel = sel; break; } }
		...
		const cands = [...nav.querySelectorAll('a')].filter((a) => a.querySelector('svg, img, .icon') && navTextSpan(a));
		...
		link.classList.remove('router-link-active', 'router-link-exact-active', 'active');
		...
		if (svg) gearInto(svg);
		else { const other = link.querySelector('img, i'); if (other) other.replaceWith(newGear(other)); }
		link.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); openSettings(); });
	...
	function watchSettingButton() {
		let scheduled = false;
		const tick = () => { scheduled = false; try { addSettingButton(); } catch (e) { console.error('LuoguSP setting entry:', e); } };
		new MutationObserver(() => { if (!scheduled) { scheduled = true; requestAnimationFrame(tick); } })
	...
	function removeAd() {
		const kill = () => {
			...
			if (ad) { ad.remove(); return true; }
			return false;
		};
		if (kill()) return;
		// 广告可能懒加载：短暂观察，移除后或 15s 后即断开，避免常驻 observer。
		const obs = new MutationObserver(() => { if (kill()) obs.disconnect(); });
	...
	function addMessageLink() {
		...
		const openUser = (uid) => { if (uid) window.open(`/user/${uid}`, '_blank'); };
		...
		async function getUidByName(username) {
			...
			try {
				const res = await fetch(`/api/user/search?keyword=${encodeURIComponent(username)}`);
		...
		function bindAvatar(img) {
			if (bound.has(img) || inUserLink(img) || !AVATAR_RE.test(img.src || '')) return;
			bound.add(img);
	...
	function digIntro(obj, wantUid) {
		...
		(function walk(o, depth) {
			...
			if (String(o.uid) === wantUid && typeof o.introduction === 'string') { result = o.introduction; return; }
	...
	async function getIntroduction(uid) {
		...
		for (const s of document.querySelectorAll('script')) {
			...
			try { const intro = digIntro(JSON.parse(t), uid); if (intro != null) return intro; } catch (e) { /* 非纯 JSON，跳过 */ }
		}
		...
		} catch (e) { console.error('LuoguSP intro fetch:', e); }
		return null;
	}
	...
	function renderMarkdown(md) {
		...
		const tt = (f, d) => { if (!kx) return null; try { return kx.renderToString(f, { throwOnError: false, displayMode: d }); } catch (e) { return null; } };
		const math = [];
		const hold = (h) => `%%LGMATH${math.push(h) - 1}%%`;  // 数学占位：纯文本，过 marked/DOMPurify 不变，最后回填
		const src = md
			.replace(/\$\$([\s\S]+?)\$\$/g, (m, f) => { const h = tt(f.trim(), true); return h ? hold(h) : m; })
			.replace(/(?<!\\)\$([^\n$]+?)\$/g, (m, f) => { const h = tt(f, false); return h ? hold(h) : m; });
		let html;
		try { html = mk.parse(src, { gfm: true, breaks: true }); } catch (e) { return renderMarkdownLite(md); }
	...
	function renderMarkdownLite(md) {
		...
		const kx = (typeof window !== 'undefined' && window.katex) || (typeof katex !== 'undefined' && katex) || null;
		const tex = (f, display) => { if (!kx) return null; try { return kx.renderToString(f, { throwOnError: false, displayMode: display }); } catch (e) { return null; } };
		...
		const inline = (s) => s
			...
			.replace(/!\[([^\]]*)\]\(([^)\s]+)[^)]*\)/g, (m, a, u) => { const x = url(u); return x ? `<img src="${x}" alt="${a}" style="max-width:100%">` : m; })
			.replace(/\[([^\]]+)\]\(([^)\s]+)[^)]*\)/g, (m, t, u) => { const x = url(u); return x ? `<a href="${x}" target="_blank" rel="noopener noreferrer">${t}</a>` : m; })
			.replace(/~~([^~]+)~~/g, '<del>$1</del>')
			.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>')
			.replace(/(^|[^*])\*([^*\n]+)\*/g, '$1<em>$2</em>');
		const cells = (row) => row.trim().replace(/^\||\|$/g, '').split('|').map((c) => c.trim());
		const table = (lines) => `<table><thead><tr>${cells(lines[0]).map((c) => `<th>${inline(c)}</th>`).join('')}</tr></thead><tbody>${lines.slice(2).map((l) => `<tr>${cells(l).map((c) => `<td>${inline(c)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
		const SAFE = /^(b|strong|i|em|u|s|del|ins|mark|sub|sup|br|hr|code|kbd|small)$/i; // 允许原样的安全内联标签
		// esc 前抽出原样片段：① 围栏代码 ② 白名单裸 HTML（洛谷允许少量 HTML）③ 数学公式（含 <>&\ 会被 esc 破坏）
		let src = md
			.replace(/```([^\n]*)\n?([\s\S]*?)```/g, (_, rawLang, c) => {
				const cls = codeLanguageClass(rawLang);
				return stash(`<pre${cls}><code${cls}>${esc(c.replace(/\n$/, ''))}</code></pre>`);
			})
			.replace(/<img\b[^>]*>/gi, (t) => { const x = url(ga(t, /\bsrc\s*=\s*["']?([^"'\s>]+)/i)); return x ? stash(`<img src="${esc(x)}" alt="${esc(ga(t, /\balt\s*=\s*["']([^"']*)["']/i))}" style="max-width:100%">`) : ''; })
			.replace(/<a\b[^>]*>/gi, (t) => { const x = url(ga(t, /\bhref\s*=\s*["']?([^"'\s>]+)/i)); return stash(x ? `<a href="${esc(x)}" target="_blank" rel="noopener noreferrer">` : '<span>'); })
			.replace(/<\/a>/gi, () => stash('</a>'))
			.replace(/<(\/?)([a-z][a-z0-9]*)\b[^>]*>/gi, (t, sl, nm) => SAFE.test(nm) ? stash(`<${sl}${nm.toLowerCase()}>`) : t) // 白名单标签去所有属性防 on*；其余留原样→后面 esc 成文本
			.replace(/\$\$([\s\S]+?)\$\$/g, (m, f) => { const h = tex(f.trim(), true); return h ? stash(h) : m; })
			.replace(/(?<!\\)\$([^\n$]+?)\$/g, (m, f) => { const h = tex(f, false); return h ? stash(h) : m; });
		...
		const html = src.split(/\n{2,}/).map((block) => {
			...
			if (lines.length >= 2 && /^\s*\|.*\|\s*$/.test(lines[0]) && /^\s*\|[\s:|-]+\|\s*$/.test(lines[1])) return table(lines);
			const h = b.match(/^(#{1,6})[ \t]+(.+)$/);
			...
			if (lines.every((l) => /^&gt;\s?/.test(l))) return `<blockquote>${inline(lines.map((l) => l.replace(/^&gt;\s?/, '')).join('<br>'))}</blockquote>`;   // > 已被 esc 转成 &gt;
	...
	async function showHiddenIntro() {
		...
		const place = () => {
			if (document.querySelector('.introduction:not(.luogusp-intro-card *)')) return true; // 原生简介已出现（管理员等）→ 别补
	...
	class FetchRateLimiter {
		...
		_drain() {
			while (this.active < this.concurrency && this.queue.length) {
				...
				if (wait > 0) { setTimeout(() => this._drain(), wait); return; }  // 距上次发起未满 launchGap
				this.nextAt = Date.now() + this.launchGap;
				const job = this.queue.shift();
				this.active++;
				fetch(job.url).then(job.resolve, job.reject).finally(() => { this.active--; this._drain(); });
			}
		}
		fetchText(url) {
	...
	async function fetchDifficulty(pid) {
		if (contentOnlyOk !== false) {
			let text;
			try { text = await limiter.fetchText(`/problem/${pid}?_contentOnly=1`); }
			catch (e) { /* 网络错误，别据此判定接口不支持 */ }
			if (text != null) {
				try {
					const d = JSON.parse(text)?.currentData?.problem?.difficulty;
					if (typeof d === 'number') { contentOnlyOk = true; return d; }
				} catch (e) {
	...
	function cacheDifficulty(pid, difficulty) {
		if (pid && typeof difficulty === 'number') colorCache.set(pid, diffColor(difficulty));
	}
	function harvestFromFeInstance() {
		...
		if (url.startsWith('https://www.luogu.com.cn/record/list')) {
			...
			if (list && !list._luogusp) {
				for (const it of list) cacheDifficulty(it.problem && it.problem.pid, it.problem && it.problem.difficulty);
				list._luogusp = true;
			}
		}
	...
	async function colorAnchor(a) {
		...
		let isForum = false;
		if (pid.includes('?forum=')) { pid = pid.split('=').pop(); isForum = true; }
		...
		if (span && span.matches('span.pid') && span.innerText === pid) {
			...
			if (color) { span.style.color = color; span.style.fontWeight = 'bold'; a.dataset.luoguspPid = pid; }
		} else {
			const color = await getProblemColor(pid);
			...
			for (const m of muts) {
				if (m.type === 'childList') {
					for (const n of m.addedNodes) {
						...
						if (n.querySelectorAll) n.querySelectorAll('a[href]').forEach(colorAnchor);
					}
	...
	const FEATURES = [
		...
		{ key: `${STORAGE_PREFIX}addProblemsColor`, run: () => setTimeout(addProblemsColor, 500) },
		{ key: `${STORAGE_PREFIX}removeAd`,         run: removeAd },
		{ key: `${STORAGE_PREFIX}addMessageLink`,   run: () => { if (isChatPage) setTimeout(addMessageLink, 500); } },
	...
	for (const f of FEATURES) {
		...
		try { f.run(); } catch (e) { console.error('LuoguSP feature failed:', e); }
	}
})();
