...
[![Version: 2.12.2](https://img.shields.io/badge/version-2.12.2-2f80ed.svg?style=flat-square)](LuoguSP.user.js)
...
- 受限文章/剪贴板就地显示：国内站访问受限的专栏文章与云剪贴板（「安全访问中心」拦截页）时，以加载动画遮住跳转确认页，读取[洛谷保存站](https://www.luogu.me/)存档并注入洛谷官方页面壳；作者信息、正文、只读评论区、扩展按钮与时间信息渲染就绪后立即显示页面。未收录内容首次访问会自动发起收录，已收录内容可点「申请更新」手动刷新；点赞收藏等互动仅国际站可用（显示为 `-`）。
- 页面内设置：在洛谷导航中加入插件设置入口，可开关各项功能。
- SPA 兼容：洛谷站内路由切换后会自动补充入口和个人简介。

## 兼容性与维护

- 脚本仅匹配 `https://www.luogu.com.cn/*`。受限文章/剪贴板候选路径会在 `document-start` 建立首屏加载遮罩，其余功能仍在 DOM 加载完成后启动。
